...
class EdgeDiscontinuityAnalyzer:
    """Detects rectangular cut-and-paste boundary halos, ignoring official template form lines."""
    ...
    def audit(cls, img_cv: np.ndarray, qr_bbox: list = None) -> dict:
        ...
        # Pristine white background detection
        white_pixels = gray > 240
        is_pristine_digital_doc = (np.sum(white_pixels) / max(total_area, 1)) > 0.35

        # Digital vector forms contain legitimate 1px form boxes; suppress them
        if is_pristine_digital_doc:
            return {"tamper_zones": [], "count": 0, "mean_gradient": 12.0}
        ...
        high_thresh = max(120, min(int(mean_g + (2.7 * std_g)), 165))
        ...
        bridged = cv2.morphologyEx(clean_mask, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT, (4, 4)))
        contours, _ = cv2.findContours(bridged, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        tamper_zones = []

        for cnt in contours:
            area = cv2.contourArea(cnt)
            if 150 < area < (total_area * 0.08):
